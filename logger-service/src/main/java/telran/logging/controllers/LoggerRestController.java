...
import static telran.logging.api.ApiConstants.*;
...
List<LogDto> getLogsDates(@RequestParam (name=FROM, defaultValue = FROM_DEFAULT_VALUE) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime from,
...
//   LOG.debug("from: {} to: {}",from, to);
    ...
    List<LogDto> getExceptionsType(@RequestParam (name=FROM, defaultValue = FROM_DEFAULT_VALUE) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime from,
                                   @RequestParam (name=TO, defaultValue = TO_DEFAULT_VALUE) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime to,
                                   @RequestParam (name=EXCEPTION_TYPE, required = false) ExceptionType exceptionType){
    	
    return exceptionType == null ? service.getAllExceptions(from, to) : service.getExceptionsType(from,to,exceptionType);
    }
    ...
    List<LogDto> getExceptionsType(@RequestParam (name=FROM, defaultValue = FROM_DEFAULT_VALUE) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime from,
                                   ...
                                   @RequestParam (name=METHOD_NAME) String methodName){
